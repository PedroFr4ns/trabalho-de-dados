...
DEFAULT_RELATIVE = Path(__file__).resolve().parent / "Health_Risk_Dataset.csv"
...
st.write("CSV default:", DEFAULT_RELATIVE)
st.write("CSV existe?", DEFAULT_RELATIVE.exists())
...
if file_mode == "Enviar um CSV":
    ...
    # Tenta caminho padrão (mesma pasta deste .py), com fallback para caminho informado
    path_text = st.sidebar.text_input(
        "Caminho do CSV (opcional)",
        value=str(DEFAULT_RELATIVE),
        help="Por padrão, procura por 'Health_Risk_Dataset.csv' na mesma pasta deste arquivo.",
    )
...
# === DISPERSÃO ===
elif analysis == "Dispersão (scatter)":
